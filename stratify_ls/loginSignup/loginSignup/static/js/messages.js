...
// let endpoint = wsStart + loc.host + '/ws/chat/'; // Add 'ws' in path.
...
socket.onopen = function (e) {
  ...
  send_message_form.on('submit', function(e){
        ...
        // let thread_id_element = $('.messages-wrapper.is_active');

        // if (!thread_id_element.length) {
        //     console.error("Error: No active chat found.");
        //     return;
        // }

        // let thread_id = thread_id_element.attr('chat-id');
        // if (!thread_id) {
        //     console.error("Error: chat-id attribute not found.");
        //     return;
        // }

        // thread_id = thread_id.split('_')[1];
        // if (!thread_id) {
        //     console.error("Error: thread_id is not valid.");
        //     return;
        // }

        if(USER_ID ==2) // ubah kalau yg dicari == ? -> send to that user
        {
            send_to = 3;
        }
        else{
...
// var socket = new WebSocket(endpoint) // Use endpoint instead of hardcoded url.
...
socket.onmessage = async function(e) {
    // console.log('message', e) // on message in console
    ...
    if(!thread_id){
        console.error("Error : thread_id not defined in received data");
        return;
    }
    newMessage(message, sent_by_id, thread_id)

    // Tambahkan pesan ke chat yang sesuai
    // let chat_id = `chat_${sent_by}`;
    // let chat_box = $(`[chat-id="${chat_id}"] .msg_card_body`);
    ...
    // } else {
    //     console.log("Chat box not found for chat ID:", chat_id);
    // }
}
...
function newMessage(message, sent_by_id) {
    console.assert('New message: ', message, 'sent by: ', sent_by_id);
	if ($.trim(message) === '') {
		return false;
	}
	// let message_element;
