// console.log('messages.js loaded');
// let loc = window.location
// let wsStart ='ws://'

// // comparing protocol
// if(loc.protocol === 'https'){
//     wsStart = 'wss://'
// }
// let endpoint = wsStart + loc.host + loc.pathname

// var socket = new WebSocket(endpoint)

// socket.onopen = async function(e){
//     console.log('open', e)
// }

// socket.onmessage = async function(e) {
//     console.log('message', e)
// }

// socket.onerror = async function(e) {
//     console.log('error', e)
// }

// socket.onclose = async function(e) {
//     console.log('close', e)
// }




// // let input_message = $('#input-message')
// // let message_body = $('.msg_card_body')
// // let send_message_form = $('#send-message-form')
// // const USER_ID = $('#logged-in-user').val()

// // let loc = window.location
// // let wsStart = 'ws://' //web socket

// // if(loc.protocol === 'https') {
// //     wsStart = 'wss://'
// // }
// // let endpoint = wsStart + loc.host + loc.pathname

// // var socket = new WebSocket(endpoint)

// // const chatSocket = new WebSocket('ws://' + window.location.host + '/ws/chat/');

// // // Event handler untuk WebSocket
// // chatSocket.onopen = function (e) {
// //     console.log('WebSocket connection established');
// // };

// // chatSocket.onerror = function (error) {
// //     console.error('WebSocket error:', error);
// // };

// // chatSocket.onclose = function (e) {
// //     console.log('WebSocket connection closed:', e);
// // };

// // chatSocket.onmessage = function (e) {
// //     const data = JSON.parse(e.data);
// //     console.log('Message received:', data);

// //     // Tambahkan logika untuk menangani pesan yang diterima
// // };

// // socket.onopen = async function(e){
// //     console.log('open', e)
// //     send_message_form.on('submit', function (e){
// //         e.preventDefault()
// //         let message = input_message.val()
// //         let send_to;

// //         if(USER_ID==1)
// //             send_to =2
// //         else
// //             send_to = 1

// //         // let send_to = get_active_other_user_id()
// //         // let thread_id = get_active_thread_id()

// //         let data = {
// //             'message': message,
// //             'sent_by': USER_ID,
// //             'send_to':send_to,
// //             'thread_id': thread_id,

// //             // 'sent_by': USER_ID,
// //             // 'send_to': send_to,
// //             // 'thread_id': thread_id
// //         }
// //         data = JSON.stringify(data)
// //         socket.send(data) //take info to consumer
// //         $(this)[0].reset()
// //     })
// // }

// // socket.onmessage = async function(e){
// //     console.log('message', e)
// //     let data = JSON.parse(e.data)
// //     let message = data['message']
// //     newMessage(message) //message sended


// //     // let sent_by_id = data['sent_by']
// //     // let thread_id = data['thread_id']
// //     // newMessage(message, sent_by_id, thread_id)
// // }

// // socket.onerror = async function(e){
// //     console.log('error', e)
// // }

// // socket.onclose = async function(e){
// //     console.log('close', e)
// // }


// // // function newMessage(message, sent_by_id, thread_id) {
// // // 	if ($.trim(message) === '') {
// // // 		return false;
// // // 	}
// // // 	let message_element;
// // // 	let chat_id = 'chat_' + thread_id
// // // 	if(sent_by_id == USER_ID){
// // // 	    message_element = `
// // // 			<div class="d-flex mb-4 replied">
// // // 				<div class="msg_cotainer_send">
// // // 					${message}
// // // 					<span class="msg_time_send">8:55 AM, Today</span>
// // // 				</div>
// // // 				<div class="img_cont_msg">
// // // 					<img src="data:image/jpeg;base64," class="rounded-circle user_img_msg">
// // // 				</div>
// // // 			</div>
// // // 	    `
// // //     }
// // // 	else{
// // // 	    message_element = `
// // //            <div class="d-flex mb-4 received">
// // //               <div class="img_cont_msg">
// // //                  <img src="https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg" class="rounded-circle user_img_msg">
// // //               </div>
// // //               <div class="msg_cotainer">
// // //                  ${message}
// // //               <span class="msg_time">8:40 AM, Today</span>
// // //               </div>
// // //            </div>
// // //         `

// // //     }

// // //     let message_body = $('.messages-wrapper[chat-id="' + chat_id + '"] .msg_card_body')
// // // 	message_body.append($(message_element))
// // //     message_body.animate({
// // //         scrollTop: $(document).height()
// // //     }, 100);
// // // 	input_message.val(null);
// // // }


// // $('.contact-li').on('click', function (){
// //     $('.contacts .actiive').removeClass('active')
// //     $(this).addClass('active')

// //     // message wrappers
// //     let chat_id = $(this).attr('chat-id')
// //     $('.messages-wrapper.is_active').removeClass('is_active')
// //     $('.messages-wrapper[chat-id="' + chat_id +'"]').addClass('is_active')

// // })

// // // function get_active_other_user_id(){
// // //     let other_user_id = $('.messages-wrapper.is_active').attr('other-user-id')
// // //     other_user_id = $.trim(other_user_id)
// // //     return other_user_id
// // // }

// // // function get_active_thread_id(){
// // //     let chat_id = $('.messages-wrapper.is_active').attr('chat-id')
// // //     let thread_id = chat_id.replace('chat_', '')
// // //     return thread_id
// // // }

// // // document.querySelectorAll('.contacts').forEach(contactItem => {
// // //     contactItem.addEventListener('click', function () {
// // //         const userId = this.getAttribute('data-user-id'); // Ambil user ID dari atribut data
// // //         const username = this.querySelector('.user_info span').textContent; // Ambil username dari elemen span

// // //         // Panggil fungsi openChat
// // //         openChat(userId, username);
// // //     });
// // // });

// // // listener klik profil user

// // // document.querySelector('.contacts').addEventListener('click', function(event){
// // //     // elemen clicked
// // //     const contactItem = event.target.closest('.contact-li');

// // //     // get user id 
// // //     if (contactItem){
// // //         const userId = contactItem.getAttribute('data-user-id');
// // //         // get username element span
// // //         const username = contactItem.querySelector('.user_info span').textContent;

// // //         console.log('User ID: ', userId);

// // //         openChat(userId, username);
// // //     }

// // // })

// // // document.querySelector('.contacts').addEventListener('click', function (event) {
// // //     const contactItem = event.target.closest('.contact-li');
// // //     if (contactItem) {
// // //         const userId = contactItem.getAttribute('data-user-id');
// // //         const username = contactItem.querySelector('.user_info span').textContent;
// // //         console.log('User ID: contacts', userId); // Debugging
// // //         openChat(userId, username);
// // //     }
// // // });

// // // Deklarasi WebSocket
// // const chatSocket = new WebSocket('ws://' + window.location.host + '/chat/');

// // // Event handler untuk WebSocket
// // chatSocket.onopen = function (e) {
// //     console.log('WebSocket connection established');
// // };

// // chatSocket.onmessage = function (e) {
// //     const data = JSON.parse(e.data);
// //     console.log('Message received:', data);

// //     // Tambahkan logika untuk menampilkan pesan di UI
// //     const messageBody = document.querySelector('.msg_card_body');
// //     const messageElement = document.createElement('div');
// //     messageElement.classList.add('d-flex', 'mb-4');
// //     messageElement.innerHTML = `
// //         <div class="msg_cotainer">
// //             ${data.message}
// //             <span class="msg_time">${data.timestamp}</span>
// //         </div>
// //     `;
// //     messageBody.appendChild(messageElement);
// //     messageBody.scrollTop = messageBody.scrollHeight;
// // };

// // chatSocket.onerror = function (e) {
// //     console.error('WebSocket error:', e);
// // };

// // chatSocket.onclose = function (e) {
// //     console.log('WebSocket connection closed:', e);
// // };

// // // Mengirim pesan
// // document.querySelector('#send-message-form').addEventListener('submit', function (e) {
// //     e.preventDefault();
// //     const message = document.querySelector('#input-message').value;
// //     const data = JSON.stringify({
// //         message: message,
// //         sent_by: USER_ID,
// //         send_to: 2, // Ganti dengan ID penerima
// //     });
// //     chatSocket.send(data);
// //     document.querySelector('#input-message').value = '';
// // });

// // document.querySelector('.contacts').addEventListener('click', function (event) {
// //     console.log('Clicked element:', event.target); // Debugging
// //     const contactItem = event.target.closest('.contact-li');
// //     if (contactItem) {
// //         console.log('Contact Item:', contactItem); // Debugging
// //         const userId = contactItem.getAttribute('data-user-id');
// //         const username = contactItem.querySelector('.user_info span').textContent;
// //         console.log('User ID:', userId, 'Username:', username); // Debugging
// //         openChat(userId, username);
// //     } else {
// //         console.error('No contact-li found!');
// //     }
// // });

// // document.querySelector('.search').addEventListener('input', function () {
// //     const query = this.value;

// //     if (query.length > 0) {
// //         fetch(`/chat/search-users/?q=${query}`)
// //             .then(response => response.json())
// //             .then(data => {
// //                 const contactsList = document.querySelector('.contacts');
// //                 contactsList.innerHTML = ''; // Kosongkan daftar kontak

// //                 data.forEach(user => {
// //                     const contactItem = document.createElement('li');
// //                     contactItem.classList.add('contact-li');
// //                     contactItem.setAttribute('style', 'cursor: pointer');
// //                     contactItem.setAttribute('data-user-id', user.id); // Tambahkan user ID
// //                     contactItem.innerHTML = `
// //                         <div class="d-flex bd-highlight">
// //                             <div class="img_cont">
// //                                 <img src="https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg" class="rounded-circle user_img">
// //                                 <span class="online_icon"></span>
// //                             </div>
// //                             <div class="user_info">
// //                                 <span>${user.username}</span>
// //                                 <p>${user.email}</p>
// //                             </div>
// //                         </div>
// //                     `;
// //                     contactsList.appendChild(contactItem);

// //                     contactItem.addEventListener('click', function (){
// //                         openChat(user.id, user.username)
// //                     })
// //                 });
// //             })
// //             .catch(error => console.error('Error fetching users:', error));
// //     }
// // });

// // // // Event delegation untuk menangani klik pada .contact-li
// // // document.querySelector('.contacts').addEventListener('click', function (event) {
// // //     if (event.target.closest('.contact-li')) {
// // //         const contactItem = event.target.closest('.contact-li');
// // //         const userId = contactItem.getAttribute('data-user-id');
// // //         const username = contactItem.querySelector('.user_info span').textContent;

// // //         // Panggil fungsi openChat
// // //         openChat(userId, username);
// // //     }
// // // });




// // // open chat

// // function openChat(userId, username) {
// //     console.log('User ID:', userId); // Debugging

// //     console.log(`Looking for .messages-wrapper[other-user-id="${userId}"]`);


// //     // Nonaktifkan semua elemen messages-wrapper
// //     document.querySelectorAll('.messages-wrapper').forEach(wrapper => {
// //         wrapper.classList.add('hide');
// //         wrapper.classList.remove('is_active');
// //     });

// //     // Aktifkan elemen messages-wrapper yang sesuai

// //     const activeWrapper = document.querySelector(`.messages-wrapper[other-user-id="${userId}"]`);
    

// //     console.log('Active Wrapper:', activeWrapper); // Debugging
// //     if (activeWrapper) {
// //         activeWrapper.classList.remove('hide');
// //         activeWrapper.classList.add('is_active');
// //     } else {
// //         console.error('Active chat wrapper not found!');
// //         return;
// //     }

// //     // Update header chat
// //     const chatHeader = activeWrapper.querySelector('.msg_head .user_info span');
// //     if (chatHeader) {
// //         chatHeader.textContent = `Chat with ${username}`;
// //     } else {
// //         console.error('Chat header element not found!');
// //         return;
// //     }

// //     // Kosongkan pesan lama di chat body
// //     const messageBody = activeWrapper.querySelector('.msg_card_body');
// //     if (messageBody) {
// //         messageBody.innerHTML = '';
// //     } else {
// //         console.error('Message body element not found!');
// //         return;
// //     }

// //     // Ambil pesan dari backend untuk user yang dipilih
// //     fetch(`/chat/get-messages/?user_id=${userId}`)
// //         .then(response => response.json())
// //         .then(data => {
// //             data.messages.forEach(message => {
// //                 const messageElement = document.createElement('div');
// //                 messageElement.classList.add('d-flex', 'mb-4');
// //                 messageElement.classList.add(message.sent_by === userId ? 'replied' : 'received');
// //                 messageElement.innerHTML = `
// //                     <div class="msg_cotainer${message.sent_by === userId ? '_send' : ''}">
// //                         ${message.message}
// //                         <span class="msg_time">${message.timestamp}</span>
// //                     </div>
// //                 `;
// //                 messageBody.appendChild(messageElement);
// //             });

// //             // Scroll ke bawah untuk menampilkan pesan terbaru
// //             messageBody.scrollTop = messageBody.scrollHeight;
// //         })
// //         .catch(error => console.error('Error fetching messages:', error));
// // }