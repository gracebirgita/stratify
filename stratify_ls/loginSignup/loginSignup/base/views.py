...
def analyticsCompany(request):
    ...
    charts_list = list(selected_charts.values('title','labels', 'values'))  # Python list
    charts_json = json.dumps(charts_list)  # JSON string for JavaScript

    return render(request, 'analytics.html', {
        'charts': charts_list,         # usable in {% for chart in charts %}
        'charts_json': charts_json     # usable in <script>
    })

@login_required
def analyticsInvestor(request):
    selected_chart_ids = UserChartSelection.objects.filter(user=request.user).values_list('chart_id', flat=True)
    selected_charts = ChartData.objects.filter(id__in=selected_chart_ids)
...
def add_activity(request):
    if request.user.role != 1:
        return HttpResponseForbidden("Only companies can add activities.")
    
    if request.method == 'POST':
        form = ActivityForm(request.POST, request.FILES)
        if form.is_valid():
            activity = form.save(commit=False)
            activity.company = request.user
            activity.save()
            messages.success(request, 'Activity added successfully!')
            return redirect('view_profile', name=request.user.username)
    else:
        form = ActivityForm()
...
def analyticIndex(request):
    ...
    return render(request, 'analyticIndex.html', {
        'form': form,
        'charts': charts,
...
def view_profile(request, name):
    ...
    context['charts'] = charts_list
    context['charts_json'] = charts_json
    
    if profile_user.role == 1:  # Company
        activities = CompanyActivity.objects.filter(company=profile_user).order_by('-date')
        context['activities'] = activities
        print(f'chartList:{charts_list}')
        print(f'chartJSON: {charts_json}')
